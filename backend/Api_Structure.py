...
async def create_calendar(
    ...
    cal = Calendar(
        owner_user_id=current_user.id, name=payload.name, visibility=payload.visibility
    )
    session.add(cal)
    await session.commit()
...
async def get_calendar(
    ...
    cal = (
        await session.execute(select(Calendar).where(Calendar.id == calendar_id))
    ).scalar_one_or_none()
    if not cal:
...
async def get_event(
    ...
    cal = (
        await session.execute(select(Calendar).where(Calendar.id == ev.calendar_id))
    ).scalar_one_or_none()
    ...
    exists = (
        await session.execute(
            select(EventShare).where(
                EventShare.event_id == event_id, EventShare.user_id == payload.user_id
            )
        )
    ).scalar_one_or_none()
    if not exists:
...
async def unshare_event(
    ...
    row = (
        await session.execute(
            select(EventShare).where(
                EventShare.event_id == event_id, EventShare.user_id == user_id
            )
        )
    ).scalar_one_or_none()
    if row:
